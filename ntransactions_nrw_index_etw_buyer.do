...
gen ln_transactions_buyer = ln(n_transactions_buyer)
   ...
   preserve
	...
	save nprivate_states_transactions.dta, replace
   restore
   merge m:1 year using nprivate_states_transactions.dta
   drop _merge
   ...
   preserve
	...
	save private_transactions.dta, replace
   restore
   merge m:1 year using private_transactions.dta

*plot
    twoway ///
        (line trans_nprivate_index year, mcolor(blue) lpattern(solid)) ///
        (line trans_private_index year, mcolor(red)), ///
        legend(label(1 "Private") label(2 "Non-private") position(4) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2018q2 = 1)", size(small)) xtitle("Year", size(small)) xline(2021, lpattern(dash) lcolor(black)) xline(2023, lpattern(dash) lcolor(black)) text(1.05 2020 "2021", size(small)) text(1.05 2022 "2023", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\transaction_index_nrw_etw_buyer.pdf", replace
