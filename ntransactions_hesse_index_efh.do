...
*plot
    twoway ///
        ...
        legend(label(1 "Hesse") label(2 "Other States")position(8) ring(0) cols(1) size(small)) ytitle("Ln(transactions) index (2023h2 = 1)", size(small)) xtitle("Year half", size(small)) xline(127, lcolor(black) lpattern(dash)) text(1.2 126 "2023h2", size(small)) 
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\transaction_index_hesse_efh.pdf", replace
