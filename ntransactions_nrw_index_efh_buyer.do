...
*plot
    twoway ///
        ...
        legend(label(1 "Private") label(2 "Non-private") position(4) ring(0) cols(1) size(small)) ytitle("Ln(transactions) index (2021 = 1)", size(small)) xtitle("Year", size(small)) xline(2021, lpattern(dash) lcolor(black)) xline(2023, lpattern(dash) lcolor(black)) text(1.1 2020 "2021", size(small)) text(1.1 2022 "2023", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\transaction_index_nrw_efh_buyer.pdf", replace
