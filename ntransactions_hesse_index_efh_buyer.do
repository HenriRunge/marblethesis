...
gen ln_transactions_buyer = ln(n_transactions_buyer)
   ...
   preserve
    ...
    collapse (sum) trans_private = ln_transactions_buyer, by(year_half)
   ...
   preserve
    ...
    collapse (sum) trans_nprivate = ln_transactions_buyer, by(year_half)
...
*plot
    twoway ///
        ...
        legend(label(1 "Private") label(2 "Non-private")position(11) ring(0) cols(1) size(small)) ytitle("Ln(transactions) index (2023h2 = 1)", size(small)) xtitle("Year half", size(small)) xline(127, lcolor(black) lpattern(dash)) text(2 126 "2023h2", size(small)) 
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\transaction_index_hesse_efh_buyer.pdf", replace
