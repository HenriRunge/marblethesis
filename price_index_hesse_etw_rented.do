...
*drop if city == "Wiesbaden"
    ...
    * Compute mean price per rentstatus and half
    collapse (mean) mean_price = ln_price_sqm, by(unrented year_half)
    ...
    * Graph
    twoway (line price_index_unrented year_half, mcolor(blue) msymbol(o)) ///
           ...
           legend(label(1 "Unrented") label(2 "Rented")position(6) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2023h2 = 1)", size(small)) xtitle("Year half", size(small)) xline(127, lcolor(black) lpattern(dash)) text(1.2 126 "2023h2", size(small)) 
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_etw_unrented.pdf", replace
