...
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_etw_full_controls.pdf", replace
...
twoway ///
    ...
    legend(order(1 "NRW" 2 "Hesse" 3 "Baden-Württemberg" 4 "Bavaria" 5 "Saxony" 6 "Thuringia" 7 "Brandenburg" 8 "Schleswig-Holstein" 9 "Berlin" 10 "Hamburg")position(11) ring(0) cols(2) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.1 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_etw_comparison.pdf", replace
...
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_etw_comparison_reduced.pdf", replace
...
*Plot the two indices
twoway (line price_hesse year_quarter, mcolor(blue) msymbol(o)) (line price_other year_quarter, mcolor(red) msymbol(x)), legend(label(1 "Hesse") label(2 "Other states") position(6) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.03 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_etw_reduced_controls.pdf", replace
