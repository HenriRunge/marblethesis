...
*Drop data (more valid control group)
...
collapse(mean) trans_index_bundesland, by(year bundesland)

gen group = "other"
replace group = "Bavaria" if bundesland == "Bavaria"

collapse (mean) trans_index_bundesland, by (group year)

twoway (line trans_index year if group == "other", mcolor(red)) (line trans_index year if group == "Bavaria", mcolor(blue)), legend(order(1 "Other" 2 "Bavaria") position(6) ring(0) cols(1) size(small)) ytitle("Ln(transactions) index (2017 = 1)", size(small)) xtitle("Year", size(small)) xline(2017, lcolor(black) lpattern(dash)) xline(2020, lcolor(black) lpattern(dash)) text(1.04 2016 "2017", size(small)) text(1.04 2019 "2020", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\transaction_index_bavaria.pdf", replace
