...
drop if city == "Dortmund" | city == "Düsseldorf" | city == "Münster" | city == "Köln"
    ...
    * Graph
    twoway (line price_index_private year, mcolor(blue) msymbol(o)) ///
           ...
           legend(label(1 "Private") label(2 "Non-private") position(4) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2021 = 1)", size(small)) xtitle("Year", size(small)) xline(2021, lpattern(dash) lcolor(black)) xline(2023, lpattern(dash) lcolor(black)) text(1.05 2020 "2021", size(small)) text(1.05 2022 "2023", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_nrw_etw_buyer.pdf", replace
