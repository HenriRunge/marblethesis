...
twoway (line price_hesse year_quarter, mcolor(blue) msymbol(o)) (line price_other year_quarter, mcolor(red) msymbol(x)), legend(label(1 "Hesse") label(2 "Other states") position(6) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.05 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_mfh_full_controls.pdf", replace
...
twoway ///
    ...
    legend(order(1 "NRW" 2 "Hesse" 3 "Baden-Württemberg" 4 "Bavaria" 5 "Saxony" 6 "Thuringia" 7 "Brandenburg" 8 "Schleswig-Holstein" 9 "Berlin" 10 "Hamburg")position(11) ring(0) cols(2) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.3 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_mfh_comparison.pdf", replace
	...
	(line price_index_bundesland year_quarter if bundesland == "Bavaria", lcolor(red%70)) ///
	(line price_index_bundesland year_quarter if bundesland == "Hamburg", lcolor(black%45)), ///
    legend(order(1 "NRW" 2 "Hesse" 3 "Baden-Württemberg" 5 "Hamburg")position(6) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.1 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_mfh_comparison_reduced.pdf", replace
...
*Plot the two indices
twoway (line price_hesse year_quarter, mcolor(blue) msymbol(o)) (line price_other year_quarter, mcolor(red) msymbol(x)), legend(label(1 "Hesse") label(2 "Other states") position(6) ring(0) cols(1) size(small)) ytitle("Ln(price/sqm) index (2024q1 = 1)", size(small)) xtitle("Quarter", size(small)) xline(256, lpattern(dash) lcolor(black)) text(1.05 254 "2024q1", size(small))
graph export "C:\Users\greix-hiwi-04\Nextcloud\Henri_real_estate_tax\price_index_hesse_mfh_reduced_controls.pdf", replace
